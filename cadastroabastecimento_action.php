<?php
    ...
    if($placa && $dt_abastecimento && $km_atual && $valor && $vl_litro && $latitude && $longitude && $iduser &&  $qtd_litros) {
    ...
    $dt_abastecimento = explode('/', $dt_abastecimento);
        ...
        if($abastecimento->add($abasobj)){
            $_SESSION['flash'] = 'Cadastrado com sucesso';
            header('Location:'.$base.'/cadastroabastecimento.php');
            exit;
        }
